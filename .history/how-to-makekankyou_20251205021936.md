...
## デプロイ（参考）
- Cloudflare Pages / xserver 等で静的ファイル（`npm run build` の出力）をホスト可能

## セキュリティに関する注意（簡潔）
- 本リポジトリは教育的目的でセキュリティテスト（「荒らし」）を扱う場合があります。実行は必ず検証用環境で行ってください。

## サポート
- 問題や改善提案は Issue を立ててください。README の編集も歓迎します。
